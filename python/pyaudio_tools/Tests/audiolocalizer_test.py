...
class AudioLocalizerTest(unittest.TestCase):
    ...
    def testZipFFT(self):
        reals = [4, 0, 1, 0]
        imags = [2, 1, 0, 0]
        zipped = self.dirloc.zip_fft(reals, imags)
    ...
    def testToNumpyFormat(self):
        ...
        arr = self.dirloc.to_numpy_format(dfts)
    ...
    def testFFTComp(self):
        ...
        transformed = self.dirloc.to_numpy_format(dfts)
        ifftout = fftp.irfft(transformed[:, 0] / 2)  # Scaling in scipy is x2
        print ifftout
    ...
    def testFullFft(self):
        ...
        reals = dfts[0][0][0]
        imags = dfts[0][1][0]
        full_fft = self.dirloc.to_full_fft(reals, imags)
        print full_fft / 2
    ...
    def testRealFFT(self):
        reals = [4, 0, 1, 0]
        imags = [1, 2, 0, 0]
        half_fft = self.dirloc.to_real_fft(reals, imags)
    ...
    def testToMatlab(self):
        ...
        dfts.append(([[4, 0, 1, 0]], [[2, 1, 0, 0]]))
        dfts.append(([[1, 0, 0, 1]], [[0, 1, 1, 0]]))
        dft_arr = self.dirloc.to_matlab_format(dfts)
    ...
    def testToRealMatlab(self):
        ...
        dfts.append(([[4, 0, 1, 0]], [[2, 1, 0, 0]]))
        dfts.append(([[1, 0, 0, 1]], [[0, 1, 1, 0]]))
        dft_arr = self.dirloc.to_real_matlab_format(dfts)
    ...
    def testGetPeaksSame(self):
        ...
        delays *= AudioLocalizer.SPEED_OF_SOUND / sample_rate
    ...
    def testGetDirection3Mic(self):
        ...
        side_length = 2 * sample_delay * AudioLocalizer.SPEED_OF_SOUND / (np.sqrt(3) * sample_rate)
    ...
    def testAngularMethod(self):
        ...
        if abs(math.cos(angle)) > 1e-10:
            dist = sample_delay * AudioLocalizer.SPEED_OF_SOUND / (sample_rate * math.cos(angle))
        else:
            dist = 1
            sample_delay = 0
    ...
    def testGetDistribution3D(self):
        ...
        for n in range(N_trials):
            ...
            #d1 = d[0] - d
            delays = d / AudioLocalizer.SPEED_OF_SOUND
