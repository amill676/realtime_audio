...
class GridTrackingLocalizer(TrackingLocalizer):
  ...
  def get_distribution(self, rffts, *args):
    ...
    if len(args) == 1:
      'bla'
      d, energy = self.get_distribution_real(rffts, args[0])
    elif len(args) > 1:
      d, energy = self.get_distribution_real(rffts, args[0], *args[1:])
    else:
